...
public class CdmActionCustInfoDMWatch extends DPFTActionTableWatch {
	...
	public void postAction() throws DPFTRuntimeException {
		/*get data set from D_CDM*/
		DPFTDboSet dCdmSet = ((DPFTActionTableWatch)this.getPreviousAction()).getDataSet();
		...
		for(int i = 0; i < dCdmSet.count(); i++){
			...
			dCdm.setValue("custname", custDboSet.getCustName(dCdm.getString("customer_id")));
			dCdm.setValue("sys_no"  , custDboSet.getSysNo(dCdm.getString("customer_id")));
			dCdm.setValue("gndr"	, custDboSet.getGender(dCdm.getString("customer_id")));
		}
		
		/*set temp value to result set for next action*/
		this.setResultSet(dCdmSet);
	}
