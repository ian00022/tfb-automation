...
public class MtmActionCustInfoDMWatch extends DPFTActionTableWatch {
	...
	public void postAction() throws DPFTRuntimeException {
		/*get data set from D_MTM*/
		DPFTDboSet dMtmSet = ((DPFTActionTableWatch)this.getPreviousAction()).getDataSet();
		...
		for(int i = 0; i < dMtmSet.count(); i++){
			...
			dMtm.setValue("cname",   custDboSet.getCustName(dMtm.getString("customer_id")));
		}
		
		/*set temp value to result set for next action*/
		this.setResultSet(dMtmSet);
	}
