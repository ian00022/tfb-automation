...
public class QsmActionContactInfoDMWatch extends DPFTActionTableWatch {
	...
	public void postAction() throws DPFTRuntimeException {
		...
		/*Get Data set from "D_QSM"*/
		DPFTDboSet dQsmSet = ((DPFTActionTableWatch)this.getPreviousAction()).getDataSet();
		
		/*Set Query criteria for "O_QSM"*/
		String qString = DPFTUtil.getFKQueryString(dQsmSet.getDbo(0));
		if(qString == null){
		...
		/*Get Data set from "O_QSM"*/
		DPFTConnector connector = DPFTConnectionFactory.initDPFTConnector(config);
		DPFTOutboundDboSet oQsmSet = (DPFTOutboundDboSet) connector.getDboSet("O_QSM", qString);
		oQsmSet.load();
		
		String idStr = (String)dQsmSet.getDbo(0).getColumnValue("TREATMENT_CODE");
		...
		/*Data set from "O_QSM" should be empty*/
		if(oQsmSet.count() > 0){
		...
		String cmp_owner_email = DPFTUtil.getCampaignOwnerEmail(dQsmSet.getDbo(0).getString("camp_code"));
		String it_adm_email = TFBUtil.getMailGroup(TFBConstants.TFB_MAILGROUP_ITADM);
		long ps_start_time = System.currentTimeMillis();
		
		for(int i = 0; i < dQsmSet.count(); i++){
			String cust_id = dQsmSet.getDbo(i).getString("customer_id");
			String mobile_no = custSet.getMobile(cust_id);
			DPFTOutboundDbo new_dbo = (DPFTOutboundDbo) oQsmSet.add();
			new_dbo.setValue(dQsmSet.getDbo(i));
			DPFTDbo id_dbo = (DPFTDbo) IDSet.add();
			String id_num = String.format("%09d", i+1);
			TFBUtil.setSSMHeaderProperties(new_dbo, dQsmSet.getDbo(i), id_dbo, "S"+id_num);
			DPFTDbo dQsm = dQsmSet.getDbo(i);
			if(dQsm.isNull("mobile_priority")){
				/*use default mobile priority rule*/
				mobile_no = custSet.getPrioritizedMobilePhone(dQsm.getString("customer_id"), "QSM", GlobalConstants.DPFT_DEFAULT_PRIORITY_CODE);
			}else{
				/*use mobile_priority Setting*/
				mobile_no = custSet.getPrioritizedMobilePhone(dQsm.getString("customer_id"), "QSM", dQsm.getString("mobile_priority"));
			}
			
			//get Contact email info
			new_dbo.setValue("contactemail", TFBUtil.getSMSContactEmail(cmp_owner_email, it_adm_email));
			
			String url = dQsmSet.getDbo(i).getString("resv1");
			if(url == null){
		...
		DPFTLogger.info(this, "Processed total " + dQsmSet.count() + ", process time = " + (ps_fin_time - ps_start_time)/60000 + " min.");
		oQsmSet.setRefresh(false);
		oQsmSet.save();
		IDSet.save();
		
		/*Wrtie Usage Code to O_USAGECODE*/
		TFBUtil.processUsageCode(oQsmSet, "QSM");
		
		/*Write results to H_OUTBOUND Table*/
		TFBUtil.generateObndCtrlRecord(connector, oQsmSet, cell_code_list, cell_name_list, "QSM", true);
		oQsmSet.close();
		IDSet.close();
	}
